...
ggplot(soay, aes(x=body.size, y=fitness)) + geom_smooth()
...
sheeplm<-lm(fitness~body.size, soay)
autoplot(sheeplm)
hist(sheeplm$residuals)
...
ggplot(data = titanic,aes(x=Sex,y=Survived, color=Pclass))+geom_jitter()
...
odds # survival odds of 4:1
odds / (1 + odds)	# survival probability of 0.80


# Q: let's up our game: calculate the survival odds and probability of survival for males in class 3
odds = exp(3.2946 + 3*(-0.9606) - 2.6434)	
odds # survival odds of 1:10
odds / (1 + odds)	# survival probability of 0.097
