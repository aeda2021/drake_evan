...
lmm.data <- read.csv("../raw-data/8_mixedEffectDat.csv", header = TRUE, sep = ",")
```
...
initial.values.abund <- replicate(3, list(list(alpha = rnorm(1, 0, 30), 
  b1 = rnorm(1, 0, 30)
)))
```
...
plotPost(fm.crabs.dat$alpha, main = "Crab burrows expected at 0 salinity")
plotPost(fm.crabs.dat$b1, main = 'How salinity influences crab burrow density')
...
tick.mod <- function(){
  ...
  for(i in 1:N){
    y[i] ~ dbern(x[i])
    logit(x[i]) = b[site[i]] + b1*x1[i] + b2*x2[i] + b3*x3[i]
  }
  ## Random intercepts:
   # Now we index across site-level means
   # To declare each site-level intercept
...
datList.ticks <- list(
  ...
  site = as.numeric(as.factor(ticks$site)), # site
  n.sites = length(unique(ticks$site))# number of sites
)

initial.values.ticks <- replicate(3, list(list(b0 = rnorm(1, 0, 30), 
  b1 = rnorm(1, 0, 30),b2 = rnorm(1, 0, 30), b3 = rnorm(1, 0, 30)
)))
```
...
sapply(1:ncol(fm.ticks.dat), function(x) {
  plotPost(fm.ticks.dat[,x], main = colnames(fm.ticks.dat)[x])})
```
