...
mod1 <- lm(Sources ~ SampleDepth, ISITsub)
...
autoplot(mod1)    #It looks really bad on the residual vs. fitted plot
...
#It separates the dataset into parts, then re assigns the x values according to some criteria based on what quantile they're in
...
#The fit has improved, and it follows the data much better. But too many splits, runs the risk of over fitting
...
for(i in 1:length(mod10s)){ # Loop through each model
  mod10s[[i]] <- lm(Sources ~ SampleDepth, data=ISITsub[ISITsub$split10==i,]) # Fit a linear regression to the appropriate subset of the data.
...
# The 10-model fit follows the data extremely well, but the confidence intervals are huge.
# The 1-model fit is the opposite, where the line doesn't match the data, but the confidence intervals are decent
# 10-model predicts closest to the truth, but has worst variance.
#It appears you must compromise between model fit, and confidence in your fit. This is the bias variance tradeoff.
...
# I think they prefer cooler waters. I think so because they are most commonly found at higher latitudes, away from the tropics.
...
ggplot(spdata, aes(x=SBT.actual, y=presfit01)) +
  geom_point()
#They are present (presfit01 = 1) from a temperature range ~ (-2  -  18)
...
plot(mod2)
gam.check(mod2)
#It appears that this model is performing much better than the gam1 model.
#The p value provided in gam.check is higher too, and thats a good thing this time.
...
#SBT.actual term is significant
# I don't think it will be very wiggly
...
#It does look a bit overfit
#There's no particular reason we would expect there to be relative maximums around 2 degrees and 8 degrees
#I would instead expect there to be a gradual increase and decrease across the range of habitable temperatures.

# Q13: This dataset is actually a concatenation of seven different surveys which each have slightly different abilities to catch cod.
#Fit a new GAM that includes a categorical predictor for survey (the region vector). 
#Based on AIC, which model would you choose? How confident would you be?
gam3 = gam(presfit01~ s(SBT.actual) + region, data = spdata, family = binomial)
AIC(gam1,mod2,gam3)
gam.check(gam3)
summary(gam3)
#Based on AIC, I would choose the model with the region predictor, and I would be very confident.