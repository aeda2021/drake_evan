...
#yes, they are reasonably linear on the plot
...
#The two parameters are significantly correlated
...
augmented_car_mod_pred <- augment(car_mod, interval = 'predict')
ggplot(augmented_car_mod_pred, aes(x = wt, y = mpg)) + 
  geom_point() + 
  geom_line(aes(y = .fitted)) +  # augment uses .whatever for all the new columns it creates to avoid confusion with existing variables
  geom_ribbon(aes(ymin =.lower, ymax = .upper),alpha=0.3)  # alpha makes things transparent
#copy the augment command and change "confidence" to "predict". 
...
# you lose ~5 mpg per ton of weight on the car
...
#the reference group has changed
...
# units of intercept are units of Y variable (eggs)
# units of slope are units of Y variable per unit of continuous X variable (eggs/density)
...
#y = 2.66 + (-0.81*Season) + (-0.03*Density)
...
#y = 1.66 + (-0.81*Season) + (-0.03*Density)
...
ggplot(mtcars, aes(x = hp_standardized, y = mpg)) + 
  geom_point() + 
  geom_abline(intercept = coef(car_mod5)[1], slope = coef(car_mod5)[2]) +  # check environment/data/car_mod4 to confirm we filled in the right values for coefficients
  geom_vline(xintercept = 0, linetype="dashed") # this just draws a line at 0, for reference
